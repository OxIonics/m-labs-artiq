...
class IIR(Module):
    ...
    def __init__(self, w, w_i, w_o, t_cycle):
        ...
        self.done = Signal()
        # these should not be accessed from the driver, but
        # can be linked to a different gateware module
        self.en_iir_overrides = Signal(w_o.channels)
        self.en_out_overrides = Signal(w_o.channels)

        ###

        # pivot arrays for muxing
        profiles = Array([ch.profile for ch in self.ctrl])
        en_outs = Array([ch.en_out | en_override for ch, en_override in zip(
            self.ctrl, self.en_out_overrides)])
        en_iirs = Array([ch.en_iir | en_override for ch, en_override in zip(
            self.ctrl, self.en_iir_overrides)])
